...
df2_OG = pd.io.parsers.read_csv(prefix+'SDSS_predictors_all_flags_plus_segmap.txt', sep='\t')
df2 = df2_OG
...
if contours:
    ...
    print('df major', df_major)
    print(df_major['Gini'].values)
...
#                        [0.47,-0.96,1.84,0.49,0.42,0.16]]
    ...
    for g in range(len(gal_predictors)):
        ...
        ax2.annotate(letters[g],xycoords='data',xy=(gal_predictors[g][1],gal_predictors[g][0]), size=15, color=gal_colors[g], zorder=101)
    ...
    ax2.annotate('Total # of galaxies = '+str(len(df2_OG))+'\n             # selected = '+str(len(df2_filtered)), xy=(0.4, 1.02), xycoords='axes fraction')
    plt.savefig(prefix + '../Figures/gini_m20_contours_overplot.png', dpi=1000)
    ...
    for g in range(len(gal_predictors)):
        ...
        ax2.annotate(letters[g],xycoords='data',xy=(gal_predictors[g][3],gal_predictors[g][2]), size=15, color=gal_colors[g], zorder=101)
    

    ax2.set_ylim(xmin_c, xmax_c)
    ax2.set_xlim( ymin_c, ymax_c)
    #plt.colorbar(im2, fraction=0.046)
    ax2.set_xlabel(r'Asymmetry (A)')
    ax2.set_ylabel(r'Concentration (C)')
    ax2.set_aspect((ymax_c-ymin_c)/(xmax_c-xmin_c))
    ax2.axvline(x=0.3, ls='--', color='black')
    plt.savefig(prefix + '../Figures/C_A_contours_overplot.png', dpi=1000)
    ...
    for g in range(len(gal_predictors)):
        ...
        ax2.annotate(letters[g],xycoords='data',xy=(gal_predictors[g][5],gal_predictors[g][4]), size=15, color=gal_colors[g], zorder=101)
    

    ax2.set_ylim(xmin_n, xmax_n)
    ax2.set_xlim( ymin_n, ymax_n)
    #plt.colorbar(im1, fraction=0.046, pad=-0.13)#, label=r'# of galaxies in bin')
    plt.colorbar(im2, fraction=0.046, pad=0.1, label=r'# of galaxies in bin')
    ax2.set_xlabel(r'Shape Asymmetry ($A_S$)')
    ax2.set_ylabel(r'Sersic n')
    ax2.set_aspect((ymax_n-ymin_n)/(xmax_n-xmin_n))
    ax2.axvline(x=0.3, ls='--', color='black')
    plt.tight_layout()
    plt.savefig(prefix + '../Figures/A_S_n_contours_overplot.png', dpi=1000, bbox_inches=0) 

    STOP
    

    # First, plot full (not caring about flags)
    sns.set_style('white')
    plt.clf()
    fig = plt.figure()                                                                                                 
    ax2=fig.add_subplot(111)                                                                                                 
    dashed_line_x=np.linspace(-0.5,-3,100)
    dashed_line_y=[-0.14*x + 0.33 for x in dashed_line_x]
    heatmap, xedges, yedges, binnumber = scipy.stats.binned_statistic_2d(df2['Gini'].values, df2['M20'].values, df2['Gini'].values,statistic='count', bins=100)
    ax2.plot(dashed_line_x, dashed_line_y, ls='--', color='black')
    ...
    ymin = yedges[0]
    ymax = yedges[-1]
    im2 = ax2.imshow(np.fliplr(np.flipud(heatmap)), cmap='Greys_r',extent=[ymax, ymin, xmin, xmax], norm=matplotlib.colors.LogNorm(vmax=max_n))
    sns.kdeplot(data=df_sims, x="M20", y = "Gini", ax = ax2, color='black') 
    ax2.set_ylim(xmin, xmax)
    ax2.set_xlim(ymax, -3)
    ax2.set_xlabel(r'$M_{20}$')
    ...
    ax2.set_aspect((ymax+3)/(xmax-xmin))
    #ax2.set_aspect((ymax-ymin)/(xmax-xmin))
    ax2.annotate('Total # of galaxies = '+str(len(df2)), xy=(0.4, 1.02), xycoords='axes fraction')
    plt.savefig('../Figures/gini_m20_full_sample_contours.png', dpi=1000)

    fig = plt.figure()                                                                                                 
    ax2=fig.add_subplot(111)                                                                                                 
    heatmap, xedges, yedges, binnumber = scipy.stats.binned_statistic_2d(df2['Concentration (C)'].values, df2['Asymmetry (A)'].values, df2['Concentration (C)'].values,statistic='count', bins=100)
    xmin_c = xedges[0]
    xmax_c = xedges[-1]
    ymin_c = yedges[0]
    ymax_c = yedges[-1]
    im2 = ax2.imshow(np.fliplr((heatmap)), cmap='Greys_r',extent=[ymax_c, ymin_c, xmax_c, xmin_c], norm=matplotlib.colors.LogNorm(vmax=max_n))
    sns.kdeplot(data=df_sims, x="Asymmetry (A)", y = "Concentration (C)", ax = ax2, color='black') 
    ...
    ax2.set_aspect((ymax_c-ymin_c)/(xmax_c-xmin_c))
    ax2.axvline(x=0.3, ls='--', color='black')
    plt.savefig('../Figures/C_A_full_sample_contours.png', dpi=1000)            

    fig = plt.figure()                                                                                                 
    ax2=fig.add_subplot(111)                                                                                                 
    heatmap, xedges, yedges, binnumber = scipy.stats.binned_statistic_2d(df2['Sersic N'].values, df2['Shape Asymmetry (A_S)'].values, df2['Sersic N'].values,statistic='count', bins=100)
    xmin_n = xedges[0]
    xmax_n = xedges[-1]
    ymin_n = yedges[0]
    ymax_n = yedges[-1]
    im2 = ax2.imshow(np.fliplr((heatmap)), cmap='Greys_r',extent=[ymax_n, ymin_n, xmax_n, xmin_n], norm=matplotlib.colors.LogNorm(vmax=max_n))
    sns.kdeplot(data=df_sims, x="Shape Asymmetry (A_S)", y = "Sersic N", ax = ax2, color='black') 

    ax2.set_ylim(xmin_n, xmax_n)
    ax2.set_xlim( ymin_n, ymax_n)
    plt.colorbar(im2, fraction=0.046, label=r'# of galaxies in bin')
    ax2.set_xlabel(r'Shape Asymmetry ($A_S$)')
    ax2.set_ylabel(r'Sersic n')
    ax2.set_aspect((ymax_n-ymin_n)/(xmax_n-xmin_n))
    ax2.axvline(x=0.3, ls='--', color='black')
    plt.savefig('../Figures/A_S_n_full_sample_contours.png', dpi=1000, bbox_inches=0) 


    # Second, care about flags
    df2_nodup = df2_filtered.duplicated()
    df2 = df2_filtered[~df2_nodup]
    ...
    im2 = ax2.imshow(np.fliplr(np.flipud(heatmap)), cmap='magma_r', extent=[-3, ymax, xmax, xmin], norm=matplotlib.colors.LogNorm(vmax=max_n))#
    #extent=[ymax_c, ymin_c, xmax_c, xmin_c]
    sns.kdeplot(data=df_sims, x="M20", y = "Gini", ax = ax2, color='black') 
    
    print('for Gini what are the mins and maxes?')
    print('xmin', xmin, 'xmax', xmax)
    print('ymin', ymin, '(we choose -3)', 'ymax', ymax)
    ax2.set_ylim(xmin, xmax)
    ax2.set_xlim(ymax, -3)
    #plt.colorbar(im2, fraction=0.046, label=r'# of galaxies in bin')
    ax2.set_xlabel(r'$M_{20}$')
    ...
    #ax2.set_aspect((ymax-ymin)/(xmax-xmin))
    ax2.annotate('Total # of galaxies = '+str(len(df2)), xy=(0.4, 1.02), xycoords='axes fraction')
    plt.savefig('../Figures/gini_m20_full_sample_no_flags_contours.png', dpi=1000) 
    ...
    im2 = ax2.imshow(np.fliplr((heatmap)), cmap='magma_r',extent=[ymax_c, ymin_c, xmax_c, xmin_c], norm=matplotlib.colors.LogNorm(vmax=max_n))
    sns.kdeplot(data=df_sims, x="Asymmetry (A)", y = "Concentration (C)", ax = ax2, color='white') 
    ...
    ax2.set_aspect((ymax_c-ymin_c)/(xmax_c-xmin_c))
    ax2.axvline(x=0.3, ls='--', color='black')
    plt.savefig('../Figures/C_A_full_sample_no_flags_contours.png', dpi=1000)   
    ...
    heatmap, xedges, yedges, binnumber = scipy.stats.binned_statistic_2d(df2['Sersic N'].values, df2['Shape Asymmetry (A_S)'].values, df2['Sersic N'].values,statistic='count', bins=100,
        range = [[xmin_n, xmax_n], [ymin_n, ymax_n]])
    im2 = ax2.imshow(np.fliplr((heatmap)), cmap='magma_r',extent=[ymax_n, ymin_n, xmax_n, xmin_n], norm=matplotlib.colors.LogNorm(vmax=max_n))
    sns.kdeplot(data=df_sims, x="Shape Asymmetry (A_S)", y = "Sersic N", ax = ax2, color='white') 
    ...
    ax2.set_aspect((ymax_n-ymin_n)/(xmax_n-xmin_n))
    ax2.axvline(x=0.3, ls='--', color='black')
    plt.savefig('../Figures/A_S_n_full_sample_no_flags_contours.png', dpi=1000, bbox_inches=0)      
...
ymin = yedges[0]
ymax = yedges[-1]
im2 = ax2.imshow(np.fliplr(np.flipud(heatmap)), cmap='magma_r',extent=[ymax, ymin, xmin, xmax], norm=matplotlib.colors.LogNorm(vmax=max_n))
ax2.set_ylim(xmin, xmax)
ax2.set_xlim(ymax, -3)
#plt.colorbar(im2, fraction=0.046, label=r'# of galaxies in bin')
ax2.set_xlabel(r'$M_{20}$')
...
#ax2.set_aspect((ymax-ymin)/(xmax-xmin))
ax2.annotate('Total # of galaxies = '+str(len(df2)), xy=(0.4, 1.02), xycoords='axes fraction')
plt.savefig('../Figures/gini_m20_full_sample.png', dpi=1000) 
...
ax2.set_aspect((ymax_c-ymin_c)/(xmax_c-xmin_c))
ax2.axvline(x=0.3, ls='--', color='black')
plt.savefig('../Figures/C_A_full_sample.png', dpi=1000)            
...
ax2.set_aspect((ymax_n-ymin_n)/(xmax_n-xmin_n))
ax2.axvline(x=0.3, ls='--', color='black')
plt.savefig('../Figures/A_S_n_full_sample.png', dpi=1000, bbox_inches=0) 
...
dashed_line_y=[-0.14*x + 0.33 for x in dashed_line_x]
heatmap, xedges, yedges, binnumber = scipy.stats.binned_statistic_2d(df2['Gini'].values, df2['M20'].values, df2['Gini'].values,statistic='count', bins=100, 
    range = [[-3, xmax], [ymin, ymax]])
ax2.plot(dashed_line_x, dashed_line_y, ls='--', color='black')                                                                

im2 = ax2.imshow(np.fliplr(np.flipud(heatmap)), cmap='magma_r', extent=[ymax, -3, xmin, xmax],norm=matplotlib.colors.LogNorm(vmax=max_n))#extent=[ymax, -3, xmin, xmax],
ax2.set_ylim(xmin, xmax)
ax2.set_xlim(ymax, -3)
#plt.colorbar(im2, fraction=0.046, label=r'# of galaxies in bin')
ax2.set_xlabel(r'$M_{20}$')
...
#ax2.set_aspect((ymax-ymin)/(xmax-xmin))
ax2.annotate('Total # of galaxies = '+str(len(df2)), xy=(0.4, 1.02), xycoords='axes fraction')
plt.savefig('../Figures/gini_m20_full_sample_no_flags.png', dpi=1000) 
...
ax2.set_aspect((ymax_c-ymin_c)/(xmax_c-xmin_c))
ax2.axvline(x=0.3, ls='--', color='black')
plt.savefig('../Figures/C_A_full_sample_no_flags.png', dpi=1000)   
...
ax2.set_aspect((ymax_n-ymin_n)/(xmax_n-xmin_n))
ax2.axvline(x=0.3, ls='--', color='black')
plt.savefig('../Figures/A_S_n_full_sample_no_flags.png', dpi=1000, bbox_inches=0)      
